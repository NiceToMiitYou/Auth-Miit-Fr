        ...
        if ( options.droppable ) {
            $( document )
                .bind( 'dragstart', function( ev, ui ) {
                    ...
                    var e = $( _e );
                    if ( !e.parents( '.fc' )
                        .length ) { // not already inside a calendar
...
*/
	...
	------------------------------------------------------------*/
        ...
        function buildTable() {
            ...
            firstRowCellInners = bodyRows.eq( 0 )
                .find( '.fc-day > div' );
            firstRowCellContentInners = bodyRows.eq( 0 )
   ...
   contentClass
	...
	-----------------------------------------------------------*/
        ...
        function setWidth( width ) {
            ...
            weekNumberWidth = 0;
            if ( showWeekNumbers ) {
                weekNumberWidth = head.find( 'th.fc-week-number' )
                    .outerWidth();
            }
	...
	-----------------------------------------------------------------------*/
        ...
        function buildDayTable() {
            ...
            dayHeadCells = dayHead.find( 'th' )
                .slice( 1, -1 ); // exclude gutter
            dayBody = dayTable.find( 'tbody' );
            dayBodyCells = dayBody.find( 'td' )
                .slice( 0, -1 ); // exclude gutter
	...
	-----------------------------------------------------------------------*/
        ...
        function setHeight( height ) {
            ...
            var headHeight = dayBody.position()
                .top;
            var allDayHeight = slotScroller.position()
                .top; // including divider
            ...
            slotHeight = slotTable.find( 'tr:first' )
                .height() + 1; // +1 for bottom border
	...
	-----------------------------------------------------------------------*/
        ...
        function slotClick( ev ) {
            if ( !opt( 'selectable' ) ) { // if selectable, SelectionManager will worry about dayClick
                var col = Math.min( colCnt - 1, Math.floor( ( ev.pageX - dayTable.offset()
                    .left - axisWidth ) / colWidth ) );
	...
	-----------------------------------------------------------------------------*/
        ...
        coordinateGrid = new CoordinateGrid( function( rows, cols ) {
            ...
            var slotTableTop = slotContainer.offset()
                .top;
            var slotScrollerTop = slotScroller.offset()
                .top;
	...
	---------------------------------------------------------------------------------*/
      ...
      addMinutes( cloneDate( d2 ), snapMinutes )
        ...
        function reportDayClick( date, allDay, ev ) {
            trigger( 'dayClick', dayBodyCells[ dateToCell( date )
                .col ], date, allDay, ev );
        }
            ...
            eventElement.draggable( {
                ...
                stop: function( ev, ui ) {
                    ...
                    if ( revert ) {
                        ...
                        var minuteDelta = 0;
                        if ( !allDay ) {
                            minuteDelta = Math.round( ( eventElement.offset()
                                .top - getSlotContainer()
                                .offset()
                                .top ) / snapHeight ) * snapMinutes + minMinute - ( event.start.getHours() * 60 + event.start.getMinutes() );
                        }
...
-----------------------------------------------------------------------------*/
    ...
    function buildSlotSegLevels( segs ) {
        ...
        for ( i = 0; i < segs.length; i++ ) {
            ...
            for ( j = 0; j < levels.length; j++ ) {
                if ( !computeSlotSegCollisions( seg, levels[ j ] )
                    .length ) {
                    break;
                }
            }
    ...
    function compareSlotSegs( seg1, seg2 ) {
        return seg1.start - seg2.start || // earlier start time goes first
            ...
            ( seg1.event.title || '' )
            .localeCompare( seg2.event.title ); // tie? alphabetically by title
    }
...
-------------------------------------------------------------------------------------------------*/
    ...
    function compareDaySegments( a, b ) {
        return ( b.rightCol - b.leftCol ) - ( a.rightCol - a.leftCol ) || // put wider events first
            ...
            ( a.event.title || '' )
            .localeCompare( b.event.title ) // if a tie, sort by event title
    }
            ...
            $( document )
                .mousedown( function( ev ) {
                    ...
                    if ( ignore ) {
                        if ( $( ev.target )
                            .parents( ignore )
                            .length ) { // could be optimized to stop after first match
                            return;
                        }
                    }
                    unselect( ev );
